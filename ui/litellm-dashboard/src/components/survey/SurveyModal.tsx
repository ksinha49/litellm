...
const REASONS_OPTIONS = [
  ...
  },
  {
    ...
    description: "Ameritas LLM had the logging/guardrail integration we needed - Langfuse, OTEL, S3 logging, Azure Content Safety guardrails",
  },
  {
    id: "unified_api",
    label: "Unified API",
    description: "Ameritas LLM had the best OpenAI-compatible API across providers - OpenAI, Anthropic, Gemini, etc.",
  },
  {
    id: "breadth_of_models",
    label: "Breadth of Models/Providers",
    description: "Ameritas LLM had the provider + endpoint combinations we needed - /ocr endpoint with Mistral OCR, /batches endppint with Bedrock API, etc.",
  },
  {
    id: "other",
    label: "Other",
...
export function SurveyModal({ isOpen, onClose, onComplete }: SurveyModalProps) {
  ...
  const renderStepContent = () => {
    ...
    if (step === 1) {
      return (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-gray-900">Are you using Ameritas LLM at your company?</h2>
    ...
    if (step === 2 && data.usingAtCompany === true) {
      return (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-gray-900">What company are you using Ameritas LLM at?</h2>
    ...
    if (step === 3) {
      return (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-gray-900">When did you start using Ameritas LLM?</h2>
          <Radio.Group
    ...
    if (step === 4) {
      return (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-gray-900">Why did you pick Ameritas LLM over other AI Gateways?</h2>
