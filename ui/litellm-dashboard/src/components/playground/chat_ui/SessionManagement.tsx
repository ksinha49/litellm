  ...
  const getSessionDescription = () => {
    if (!responsesSessionId) {
      return useApiSessionManagement
        ? "Ameritas LLM will manage session using previous_response_id"
        : "UI will manage session using chat history";
    }

    return useApiSessionManagement
      ? "Ameritas LLM API session active - context maintained server-side"
    ...
    <div className="mb-4">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <Tooltip title="Choose between Ameritas LLM API session management (using previous_response_id) or UI-based session management (using chat history)">
