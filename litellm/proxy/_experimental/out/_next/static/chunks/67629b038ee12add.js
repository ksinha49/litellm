...
client = openai.OpenAI(
	api_key="${y||"YOUR_LITELLM_API_KEY"}",
	base_url="${_}"
)`;switch(h){case o.CHAT:{let e=Object.keys(I).length>0,i="";if(e){let e=JSON.stringify({metadata:I},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let r=C.length>0?C:[{role:"user",content:x}];t=`
import base64
...
`;break}case o.RESPONSES:{let e=Object.keys(I).length>0,i="";if(e){let e=JSON.stringify({metadata:I},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let r=C.length>0?C:[{role:"user",content:x}];t=`
import base64
