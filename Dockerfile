...
# Override litellm_proxy_extras schema.prisma and migrations with local versions.
# (litellm_proxy_extras is installed from PyPI, so its schema.prisma and migrations
#  would otherwise miss our custom schema additions like health_status.)
RUN PKG_DIR="$(python3 -c 'import litellm_proxy_extras, os; print(os.path.dirname(litellm_proxy_extras.__file__))')" && \
